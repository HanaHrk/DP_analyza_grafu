...
from utils import (
    ...
    evaluate, save_predictions_as_imgs_without_threshold,
)
import config
import wandb
...
MODEL = "u" #"aru" # "aru" # choose "aru", "ru", or "u"
NUM_SCALES = 5 # amount of scaled images you want to use you (e.g. 3: original image and two downscaled versions);

# Dataset paths
# TRAIN_IMG_DIR = os.path.join(config.paths.data, "axis_analysis", "sample_images/")
# TRAIN_MASK_DIR = os.path.join(config.paths.data, "axis_analysis", "sample_masks/")
...
EVALUATE = False # if you want to evaluate with Transkribus after every epoch
EVALUATION_DIR = "./evaluation/" # folder with Transkribus jar (trans.jar)
VAL_XML_DIR = "data/axis_analysis/val_xml/" # ground truth of validation images
...
def main():
    ...
    wandb_api_key = load_wandb_api_key(config.WANDB_API_KEY_DIR)
    ...
    for epoch in range(1, NUM_EPOCHS + 1):
        ...
        if epoch != 0 and epoch % 10 == 0:
            if EVALUATE:
                save_predictions_as_imgs_without_threshold(val_loader, model, output_dir=SAVED_IMAGES, output_xml_dir=OUTPUT_XML_DIR, device=DEVICE, evaluate=True, batch_size=BATCH_SIZE)
                ...
                save_predictions_as_imgs_without_threshold(val_loader, model, output_dir=SAVED_IMAGES,
                                                           output_xml_dir=OUTPUT_XML_DIR, device=DEVICE, evaluate=True, batch_size=BATCH_SIZE)

...
def save_predictions_as_imgs_without_threshold(loader, model, batch_size, output_dir="saved_images/",
                                               output_xml_dir="saved_test_images_xml/", device="cuda", evaluate=False):
...
def save_predictions_as_imgs(loader, model, batch_size, output_dir="saved_images/",
                             output_xml_dir="saved_test_images_xml/", device="cuda", evaluate=False):
    """Saves the current predictions of the training model as images. If random downsampling is disabled, evaluation can be run."""
    ...
    for idx, (x, y) in enumerate(loader):
        ...
        if evaluate:
            for i in range(len(preds)):
                ...
                # delete data endings
                if str.endswith(image_name, '.jpg') or str.endswith(image_name, '.JPG') or str.endswith(image_name,
                                                                                                        '.png'):
                    image_name = image_name[:len(image_name) - 4]
                elif str.endswith(image_name, '.jpeg'):
                    image_name = image_name[:len(image_name) - 5]
                ...
                combineImages(original_image_path=image_path, baseline_image_path=aImgPath_upscaled, folder=output_dir,
...
def save_test_predictions_as_imgs(loader, model, image_height, image_width, padding, output_dir="saved_test_images/",
                                  device="cuda", threshold: Union[None, float] = 0.5):

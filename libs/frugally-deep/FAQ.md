...
Why does `fdeep::model::predict_single` take and return multiple `fdeep::tensor`s and not just one tensor?
----------------------------------------------------------------------------------------------------
...
This `fdeep::model::predict_single` takes (and returns) not one `fdeep::tensor` but an `std::vector` of them (`fdeep::tensors`).
...
int main()
{
    ...
    const auto result = model.predict_single({
...
Keep in mind, giving multiple `fdeep::tensor`s to `fdeep::model::predict_single` has nothing to do with batch processing because it is not supported. However you can run multiple single predictions in parallel (see the question "Does frugally-deep support multiple CPUs?"), if you want to do that.
...
i.e., you can call `model.predict_single` on the same model instance from different threads simultaneously.
...
`fdeep::model::predict_single` is the generic prediction.
...
So why do you get different values nonetheless when running `fdeep::model::predict_single`?
...
print(input)
print(input.shape)
result = model.predict_single([input])
print(result)
...
const auto result = model.predict_single({input});
...
or raise an exception when `.predict_single` is invoked,

...
* supports inference (`model.predict_single`) not only for [sequential models](https://keras.io/getting-started/sequential-model-guide/) but also for computational graphs with a more complex topology, created with the [functional API](https://keras.io/getting-started/functional-api-guide/).
...
3) Finally load it in C++ (`fdeep::load_model(...)`) and use `model.predict_single(...)` to invoke a forward pass with your data.

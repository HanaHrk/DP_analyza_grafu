...
// check the parallel product is correct
  ...
  std::cout << "blas  _cpu         " << tblas.best(CPU_TIMER) / rep << "s  \t"
  ...
  std::cout << "eigen _cpu         " << tmt.best(CPU_TIMER) / rep << "s  \t"
  ...
  if (procs > 1) {
    ...
    std::cout << "eigen mono _cpu    " << tmono.best(CPU_TIMER) / rep << "s  \t"
  ...
  if (1. * m * n * p < 30 * 30 * 30) {
    ...
    std::cout << "lazy _cpu         " << tmt.best(CPU_TIMER) / rep << "s  \t"
              << (double(m) * n * p * rep * 2 / tmt.best(CPU_TIMER)) * 1e-9 << " GFLOPS \t(" << tmt.total(CPU_TIMER)
              << "s)\n";
  ...
  if ((NumTraits<A::Scalar>::IsComplex) && (NumTraits<B::Scalar>::IsComplex)) {
    ...
    std::cout << "\"matlab\" _cpu    " << t.best(CPU_TIMER) / rep << "s  \t"
